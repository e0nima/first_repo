...
def sdvig(track, key_num, key_word, i_sim, non_alph_counter):
    return (
        ((-1) ** track)
        * key_num
        * (ord(key_word[(i_sim - non_alph_counter) % len(key_word)]) - ord("a") + 1)
    )
    ...
    if not sym.isalpha():
        ...
        return (
            chr(
                (
                    ord(sym)
                    + sdvig(track, key_num, key_word, i_sim, non_alph_counter)
                    - ord("A")
                )
                % alph_power
                + ord("A")
...
def text_code(text, track, alph_power, key_word, key_num):
    ...
    for i in range(len(text)):
        res_text[i], non_alph_counter = encode_symbol(
            text[i], i, non_alph_counter, alph_power, track, key_word, key_num
        )
    return res_text
